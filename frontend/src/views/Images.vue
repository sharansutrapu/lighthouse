...
const pruneImages = () => {
  openConfirm('Prune Unused Images', 'Are you sure you want to prune all dangling images? This action cannot be undone.', 'warning', async () => {
    try {
      const res = await apiFetch('/api/images/prune', { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
        showToast('Success', `Pruned images. Freed ${formatBytes(data.SpaceReclaimed || 0)}`, 'success');
        fetchImages();
      } else {
